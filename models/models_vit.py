...
class F3Net(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(y.shape)
        f = self.dp(y)
        f = F.interpolate(y, size=(19, 19), mode='bilinear', align_corners=False)
        #f = self.fc(f)
...
class CViT(nn.Module):
    def __init__(self, image_size=380, patch_size=19, num_classes=2, channels=2560,
...
if __name__ == '__main__':
    ...
    path = r"E:\dataset\FF_c40\FF_df\000_003\10.png"
    img = tf (path)
    print(img.shape)
    img = img.unsqueeze(0)
    print(img.shape)
